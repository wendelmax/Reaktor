...
public class ReaktorComponentProcessor extends AbstractElementModelProcessor {
    ...
    protected void doProcess(ITemplateContext context, IModel model,
        ...
        String rootId = "react-root-" + componentName.toLowerCase() + "-" + ID_COUNTER.incrementAndGet();
        ...
        // Automatically inject global Reaktor metadata if not already present in props
        ...
        html.append("data-react-component=\"").append(escapeHtml(componentName)).append("\" ");
        if (ignoreHydration) {
            html.append("data-react-ignore=\"true\" ");
        }
        if (stateful) {
            html.append("data-react-stateful=\"true\" ");
        }
        if (preloadUrl != null && !preloadUrl.isBlank()) {
            html.append("data-react-preload=\"").append(escapeHtml(preloadUrl)).append("\" ");
        }
        if (pollInterval != null && !pollInterval.isBlank()) {
            int interval = parsePollInterval(pollInterval);
            if (interval > 0) {
                html.append("data-react-poll=\"").append(interval).append("\" ");
            }
        }
        html.append("data-react-props='").append(escapeJsonForAttr(propsJson)).append("'></");
    ...
    private boolean isReactSlot(IOpenElementTag tag) {
        return SLOT_ELEMENT.equalsIgnoreCase(getElementLocalName(tag));
    }

    private boolean isReactSlotOpenOrClose(ITemplateEvent event) {
    ...
    private String getAttributeValue(IProcessableElementTag tag, String attrName) {
        String value = tag.getAttributeValue("react", attrName);
        if (value == null) {
